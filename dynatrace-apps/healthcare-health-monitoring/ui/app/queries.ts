...
export const queries = {
  ...
  /** Peak (max) CPU across any single device — catches hot-spot spikes that avg hides */
  peakDeviceCpu: `timeseries cpu = max(healthcare.network.device.cpu.utilization), from: now()-15m
    | fieldsAdd peak_cpu = arrayMax(cpu)
    | fields peak_cpu`,
  ...
  mirthChannelHealth: `timeseries status=avg(healthcare.mirth.channel.status), errors=sum(healthcare.mirth.channel.messages.errors), received=sum(healthcare.mirth.channel.messages.received), queue=avg(healthcare.mirth.channel.queue.depth), by:{channel.name}, interval:5m
    | fieldsAdd latest_status = arrayLast(arrayRemoveNulls(status))
    ...
    | fieldsAdd error_rate = if(isNotNull(last_received) AND last_received > 0, toDouble(last_errors) / toDouble(last_received), else: 0.0)
    | fieldsAdd is_healthy = isNotNull(latest_status) AND latest_status > 0.5 AND error_rate < 0.10 AND (isNull(last_queue) OR last_queue < 50)
